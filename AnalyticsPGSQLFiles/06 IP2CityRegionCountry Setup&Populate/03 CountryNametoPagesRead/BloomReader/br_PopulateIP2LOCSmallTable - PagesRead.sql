DROP FUNCTION populate_country_function_br();
--BEGIN;
--SAVEPOINT sp1;
CREATE OR REPLACE FUNCTION populate_country_function_br()
...
FOR xyz_row IN xyz
	...
	SELECT b.country_code, b.country_name FROM public.ip2location AS b 
           WHERE (SELECT public.ip2int(d.context_ip) FROM bloomreader.pages_read AS d 
...
--ROLLBACK TO sp1;
SELECT * FROM populate_country_function_br();
commit;

select * from public.ip2location;
select * from public.ip2loc_sm_tab;
select * from bloomreader.pages_read where populate_country is not null;
select * from bloomreader.pages_read;

select * FROM bloomreader.pages_read AS a WHERE a.id ='010a3627-2357-43be-b383-6ce518961eae'; 


SELECT b.populate_country FROM public.ip2location AS b
   WHERE (SELECT ip2int(a.context_ip) FROM bloomreader.pages_read AS a 
                 WHERE a.id = '010a3627-2357-43be-b383-6ce518961eae') BETWEEN b.ip_from and b.ip_to;
		
UPDATE bloomreader.pages_read AS a 
