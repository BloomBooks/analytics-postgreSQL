...
DECLARE xyz CURSOR FOR SELECT * from bloomreader.application_installed AS a;
...
FOR xyz_row IN xyz
	...
	SELECT b.country_code, b.country_name FROM public.ip2location AS b 
           WHERE (SELECT public.ip2int(d.context_ip) FROM bloomreader.application_installed AS d 
...
select * from bloomreader.application_installed where populate_country is not null;
select * from bloomreader.application_installed;

select * FROM bloomreader.application_installed AS a WHERE a.id ='010a3627-2357-43be-b383-6ce518961eae'; 


SELECT b.populate_country FROM public.ip2location AS b
   WHERE (SELECT ip2int(a.context_ip) FROM bloomreader.application_installed AS a 
                 WHERE a.id = '010a3627-2357-43be-b383-6ce518961eae') BETWEEN b.ip_from and b.ip_to;
		
UPDATE bloomreader.application_installed AS a 
