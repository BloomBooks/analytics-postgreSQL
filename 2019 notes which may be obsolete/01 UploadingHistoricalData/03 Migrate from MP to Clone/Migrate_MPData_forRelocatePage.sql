--DROP FUNCTION migrate_MPData_Relocate_Page();

CREATE OR REPLACE FUNCTION migrate_MPData_Relocate_Page()
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomreadertest.clone_relocate_page as s  
...
-- Column: bloomreadertest.clone_relocate_page.location_uid
-- ALTER TABLE bloomreadertest.clone_relocate_page DROP COLUMN location_uid;
ALTER TABLE bloomreadertest.clone_relocate_page
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_relocate_page AS b

--SELECT * FROM migrate_MPData_Relocate_Page();
