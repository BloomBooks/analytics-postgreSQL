--DROP FUNCTION migrate_MPData_Launch();

CREATE OR REPLACE FUNCTION migrate_MPData_Launch()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_launch AS a
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.launch as s  
...
-- Column: bloomreadertest.clone_launch .location_uid
-- ALTER TABLE bloomreadertest.clone_launch DROP COLUMN location_uid;
ALTER TABLE bloomreadertest.clone_launch 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_launch AS b

--SELECT * FROM migrate_MPData_Launch();
select count(*) from bloomreadertest.clone_launch;
select count(*) from bloomapp.clone_insert_template_page;
select Count(DISTINCT(a.user_id, a.timestamp)) from bloomreadertest.clone_launch AS a;
select DISTINCT(a.user_id, a.timestamp), a.user_id, a.timestamp from bloomreadertest.clone_launch AS a;
