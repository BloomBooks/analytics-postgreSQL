--DROP FUNCTION migrate_old_data_ChangeContentLanguages();

CREATE OR REPLACE FUNCTION migrate_old_data_ChangeContentLanguages()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_change_content_languages AS a
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomreadertest.clone_change_content_languages as s  
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  loc_uid_temp );
     	--ON CONFLICT ON CONSTRAINT clone_change_content_languages_pkey DO NOTHING;
...
--SELECT * FROM migrate_old_data_ChangeContentLanguages();
select Count(*) from bloomapp.change_content_languages;
select * from bloomapp.change_content_languages;
select Count(*) from bloomreadertest.clone_change_content_languages;
select * from bloomreadertest.clone_change_content_languages;
...
select a.ip from bloomreadertest.mpdata_change_content_languages AS a 
  WHERE (a.mp_country is not null or a.region is not null or a.city is not null) and a.ip is not null;
select * from public.countryregioncitylu order by loc_uid DESC;
select * from bloomreadertest.clone_change_content_languages as a where
a.timestamp > ((SELECT MIN(timestamp) FROM bloomapp.change_content_languages
  WHERE context_library_version <> 'Segment.io') - interval '1 day')
  AND 
a.timestamp < ((SELECT MIN(timestamp) FROM bloomapp.change_content_languages
  WHERE context_library_version <> 'Segment.io') + interval '1 day') 
  ORDER BY a.timestamp ASC;
SELECT MIN(timestamp) FROM bloomapp.change_content_languages
  WHERE context_library_version <> 'Segment.io';
SELECT count(*) FROM bloomreadertest.clone_change_page_layout AS a
  WHERE a.timestamp > (SELECT max(to_timestamp(received_at)) FROM bloomreadertest.mpdata_change_page_layout);
  
  
  (current_date - interval '7 day') ;
  SELECT count(*) FROM bloomapp.change_content_languages;
select branding_project_name, count(branding_project_name) from bloomapp.change_content_languages
  GROUP BY branding_project_name;
  select * from bloomapp.change_content_languages where branding_project_name IS NULL;
select count(*) from bloomreadertest.clone_change_content_languages;
select * from bloomreadertest.clone_change_content_languages;
select count(*) from bloomreadertest.mpdata_change_content_languages;
select count(*) from bloomreadertest.mpdata_change_content_languages AS a 
    where to_timestamp(a.received_at) < (SELECT MIN(timestamp) FROM bloomapp.change_content_languages
  WHERE context_library_version <> 'Segment.io');

SELECT MIN(to_timestamp(a.received_at)) from bloomreadertest.mpdata_change_content_languages AS a;
