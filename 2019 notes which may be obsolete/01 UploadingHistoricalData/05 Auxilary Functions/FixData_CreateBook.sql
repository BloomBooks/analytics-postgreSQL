--DROP FUNCTION FixData_CreateBook();

CREATE OR REPLACE FUNCTION FixData_CreateBook()
...
--SELECT * FROM FixData_CreateBook();

select * from bloomapp.create_book AS a
	 WHERE location_uid IS NOT NULL;
select count(a.ip) from bloomapp.create_book AS a
	 WHERE location_uid IS NOT NULL;
SELECT count(a.ip) from bloomreadertest.clone_create_book AS a
	 WHERE location_uid IS NOT NULL;	 
select a.country,a.ip from bloomapp.create_book AS a
  WHERE location_uid = 1;
SELECT * from bloomapp.create_book AS a
  WHERE location_uid = 1;
SELECT a.country,a.ip from bloomreadertest.clone_create_book AS a
  WHERE location_uid = 1;
