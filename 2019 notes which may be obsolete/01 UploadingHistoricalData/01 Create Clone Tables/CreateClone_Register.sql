-- Table: bloomreadertest.clone_register

-- DROP TABLE bloomreadertest.clone_register;

CREATE TABLE bloomreadertest.clone_register
(
    ...
    CONSTRAINT clone_register_pkey PRIMARY KEY (id)
)
WITH (
    OIDS = FALSE
)
...
ALTER TABLE bloomreadertest.clone_register
    OWNER to segment;

GRANT SELECT ON TABLE bloomreadertest.clone_register TO bloomappuser;
GRANT SELECT ON TABLE bloomreadertest.clone_register TO readbloom;
GRANT ALL ON TABLE bloomreadertest.clone_register TO segment;


-- ALTER TABLE bloomreadertest.clone_register DROP COLUMN location_uid;
ALTER TABLE bloomreadertest.clone_register 
    ADD COLUMN location_uid bigint;	
	
select count(*) from bloomreadertest.clone_register ;
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_register AS b;
