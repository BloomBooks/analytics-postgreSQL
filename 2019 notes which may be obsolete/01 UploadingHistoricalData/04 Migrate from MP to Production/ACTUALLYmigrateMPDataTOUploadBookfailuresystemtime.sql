--DROP FUNCTION ACTUALmigrate_MPData_upload_book_failure_system_time();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_upload_book_failure_system_time()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_upload_book_failure_system_time AS a
...
--SELECT MIN(b.timestamp) FROM bloomapp.upload_book_failure_system_time AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.upload_book_failure_system_time as s  
...
-- Column: bloomreadertest.clone_upload_book_failure_system_time.location_uid
-- ALTER TABLE bloomreadertest.clone_upload_book_failure_system_time DROP COLUMN location_uid;
ALTER TABLE bloomapp.upload_book_failure_system_time
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_upload_book_failure_system_time AS b

--SELECT * FROM ACTUALmigrate_MPData_upload_book_failure_system_time();
select * from bloomreadertest.mpdata_upload_book_failure_system_time
select to_timestamp('1522771253');
select count(*) from bloomapp.upload_book_failure_system_time;
select count(*) from bloomreadertest.clone_upload_book_failure_system_time;
select count(*) from bloomreadertest.mpdata_upload_book_failure_system_time AS a
 where to_timestamp(a.received_at) < (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_upload_book_failure_system_time AS b);
SELECT * from bloomreadertest.clone_upload_book_failure_system_time AS a;
SELECT * from bloomreadertest.mpdata_upload_book_failure_system_time AS a
