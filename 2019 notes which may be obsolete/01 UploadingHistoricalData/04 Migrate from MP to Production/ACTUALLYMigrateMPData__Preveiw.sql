--DROP FUNCTION ACTUALmigrate_MPData_preview();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_preview()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_preview AS a
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomreadertest.clone_preview   
	( id, received_at, anonymous_id, context_user_agent, browser, browser_version,
	  book, -- user_name, book, book_title, href,
	...
	(new_id, to_timestamp(xyz_row.received_at), 
	  ...
	  public.empty_to_null(xyz_row.book), -- public.empty_to_null(xyz_row.user_name),
...
SELECT * from bloomreadertest.mpdata_preview AS a
...
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_preview AS b
	...
	SET  context_library_name = context_library_name||' via Mixpanel';
   	...
   	UPDATE bloomreadertest.clone_preview
	SET  context_library_name = context_library_name||' via Mixpanel'
	WHERE timestamp < '2017-08-10 17:04:21.861+00'; 
--SELECT * FROM ACTUALmigrate_MPData_preview();

select * from bloomreadertest.clone_preview
  WHERE POSITION('Mix' IN context_library_name)>0;
select to_timestamp('1522771253');
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_preview AS b
select count(*) from bloomreadertest.clone_preview;
select count(*) from bloomreadertest.mpdata_preview;
