--DROP FUNCTION ACTUALmigrate_MPData_DownloadedBookFailure();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_DownloadedBookFailure()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_downloaded_book_failure AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.downloaded_book_failure AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.downloaded_book_failure as s  
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  public.empty_to_null(xyz_row.title),public.empty_to_null(xyz_row.url),  --xyz_row.ip,
...
select * from bloomreadertest.clone_downloaded_book_failure;
-- Column: bloomreadertest.clone_downloaded_book_failure.location_uid
-- ALTER TABLE bloomreadertest.clone_downloaded_book_failure DROP COLUMN location_uid;
ALTER TABLE bloomapp.downloaded_book_failure
    ADD COLUMN location_uid bigint;	
--SELECT * FROM ACTUALmigrate_MPData_DownloadedBookFailure();

select * from bloomreadertest.clone_downloaded_book_failure;
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_downloaded_book_failure AS b
SELECT * from bloomreadertest.clone_downloaded_book_failure AS a
