--DROP FUNCTION ACTUALmigrate_MPData_Created();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_Created()
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomapp.created as s  
...
-- Column: bloomapp.created.location_uid
-- ALTER TABLE bloomapp.created DROP COLUMN location_uid;
ALTER TABLE bloomapp.created
    ADD COLUMN location_uid bigint;	
--SELECT * FROM ACTUALmigrate_MPData_Created();

select * from bloomapp.created;
SELECT MIN(b.timestamp) FROM bloomapp.created AS b
SELECT * from bloomapp.created AS a
