--DROP FUNCTION ACTUALmigrate_MPData_ExportToDocFormat();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_ExportToDocFormat()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_exported_to_doc_format AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.exported_to_doc_format AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomapp.exported_to_doc_format as s  
...
-- Column: bloomapp.exported_to_doc_format.location_uid
-- ALTER TABLE bloomapp.exported_to_doc_format DROP COLUMN location_uid;
ALTER TABLE bloomapp.exported_to_doc_format 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.exported_to_doc_format AS b

--SELECT * FROM ACTUALmigrate_MPData_ExportToDocFormat();
select count(*) from bloomapp.exported_to_doc_format;
