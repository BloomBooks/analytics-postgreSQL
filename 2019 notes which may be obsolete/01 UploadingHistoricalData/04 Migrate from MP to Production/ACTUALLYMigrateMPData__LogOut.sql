--DROP FUNCTION ACTUALmigrate_MPData_log_out();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_log_out()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_log_out AS a
...
SELECT MIN(b.timestamp) FROM bloomlibrary_org.log_out AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomlibrary_org.log_out as s  
...
-- Column: bloomlibrary_org.log_out.location_uid
-- ALTER TABLE bloomlibrary_org.log_out DROP COLUMN location_uid;
ALTER TABLE bloomlibrary_org.log_out
    ADD COLUMN location_uid bigint;	
	
SELECT * from bloomreadertest.mpdata_log_out AS a
...
SELECT MIN(b.timestamp) FROM bloomlibrary_org.log_out AS b
	...
	SET  context_library_name = context_library_name||' via Mixpanel';
   	...
   	UPDATE bloomreadertest.clone_log_out
	SET  context_library_name = context_library_name||' via Mixpanel'
	WHERE timestamp < '2017-08-10 17:04:21.861+00'; 
--SELECT * FROM ACTUALmigrate_MPData_log_out();

select * from bloomlibrary_org.log_out
  WHERE POSITION('Mix' IN context_library_name)>0;
select to_timestamp('1522771253');
SELECT MIN(b.timestamp) FROM bloomlibrary_org.log_out AS b
select count(*) from bloomlibrary_org.log_out;
select count(*) from bloomreadertest.mpdata_log_out;
...
SELECT * from bloomlibrary_org.log_out AS a;
SELECT * from bloomreadertest.mpdata_log_out AS a
