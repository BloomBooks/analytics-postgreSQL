--DROP FUNCTION ACTUALLYmigrate_mixpaneldataTOChangeContentLanguages();

CREATE OR REPLACE FUNCTION ACTUALLYmigrate_mixpaneldataTOChangeContentLanguages()
...
SELECT MIN(b.timestamp) FROM bloomapp.change_content_languages AS b        ------CHANGE TO THE NEXT TABLE
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomapp.change_content_languages as s               ------CHANGE TO THE NEXT TABLE
...
-- Column: bloomapp.change_content_languages.location_uid
-- ALTER TABLE bloomapp.change_content_languages DROP COLUMN location_uid;
ALTER TABLE bloomapp.change_content_languages
    ADD COLUMN location_uid bigint;	
SELECT MIN(b.timestamp) FROM bloomapp.change_content_languages AS b
SELECT * FROM ACTUALLYmigrate_mixpaneldataTOChangeContentLanguages();
...
select * from bloomapp.change_content_languages AS a
