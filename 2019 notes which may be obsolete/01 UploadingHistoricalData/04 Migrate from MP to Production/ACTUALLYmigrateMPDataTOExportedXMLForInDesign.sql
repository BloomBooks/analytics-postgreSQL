--DROP FUNCTION ACTUALmigrate_MPData_ExportedXMLforInDesign();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_ExportedXMLforInDesign()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_exported_xml_for_in_design AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.exported_xml_for_in_design AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.exported_xml_for_in_design as s  
...
-- Column: bloomapp.exported_xml_for_in_design.location_uid
-- ALTER TABLE bloomapp.exported_xml_for_in_design DROP COLUMN location_uid;
ALTER TABLE bloomapp.exported_xml_for_in_design
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.exported_xml_for_in_design AS b

--SELECT * FROM ACTUALmigrate_MPData_ExportedXMLforInDesign();
select count(*) from bloomapp.exported_xml_for_in_design;

select * from bloomreadertest.mpdata_exported_xml_for_in_design AS a
 where a.language3_iso639_name IS NOT NULL;
SELECT * from bloomreadertest.clone_exported_xml_for_in_design AS a
	ORDER BY a.timestamp;
	
SELECT * FROM (
	SELECT DISTINCT(a.user_id, a.timestamp), a.user_id, a.timestamp, a.location_uid 
	from bloomreadertest.clone_exported_xml_for_in_design AS a ORDER BY a.user_id
	)AS b;
