REVOKE SELECT ON bloomreadertest.Comprehension FROM readbloomtester;
DROP VIEW bloomreadertest.Comprehension CASCADE;

CREATE VIEW bloomreadertest.Comprehension AS
	SELECT DISTINCT a.title AS Title_Tested,
          ...
          FROM (SELECT y.title, ROUND(AVG(y.percent_right), 0) AS median_percent_right  
	 			FROM (
                    ...
                    FROM bloomreadertest.UnionQuestions AS z
                    GROUP BY z.title, z.percent_right
	 			     ) AS y
                 WHERE rows_ascending BETWEEN rows_descending - 1 AND rows_descending + 1
                 GROUP BY y.title
             ) AS x 
    INNER JOIN bloomreadertest.UnionQuestions AS a ON a.title = x.title  
...
GRANT SELECT ON bloomreadertest.Comprehension TO readbloomtester;
select * FROM bloomreadertest.Comprehension;