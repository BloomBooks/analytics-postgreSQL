REVOKE SELECT ON bloomreader.Books FROM bloomreaderuser;
DROP VIEW bloomreader.Books CASCADE;
...
CREATE VIEW bloomreader.Books AS
	SELECT a.timestamp AS BookTimeStamp,
          ...
          c.clname AS  BookLanguage, 
 		  CASE WHEN EXISTS (SELECT 'x' from 
								(SELECT * FROM bloomreader.questions_correct 
									UNION ALL
								 SELECT * FROM bloomreaderbeta.questions_correct) AS q  
...
GRANT SELECT ON bloomreader.Books TO bloomreaderuser;
select * FROM bloomreader.Books;
select * FROM bloomreader.Books AS a where a.Country NOT IN ('United States');
