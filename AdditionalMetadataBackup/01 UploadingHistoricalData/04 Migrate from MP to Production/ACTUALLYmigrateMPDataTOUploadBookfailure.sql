--DROP FUNCTION ACTUALmigrate_MPData_upload_book_failure();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_upload_book_failure()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_upload_book_failure AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.upload_book_failure AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.upload_book_failure as s  
...
-- Column: bloomapp.upload_book_failure.location_uid
-- ALTER TABLE bloomapp.upload_book_failure DROP COLUMN location_uid;
ALTER TABLE bloomapp.upload_book_failure
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.upload_book_failure AS b

--SELECT * FROM ACTUALmigrate_MPData_upload_book_failure();
select * from bloomreadertest.mpdata_upload_book_failure
select to_timestamp('1522771253');
select count(*) from bloomapp.upload_book_failure;
select count(*) from bloomreadertest.clone_upload_book_failure;
select count(*) from bloomreadertest.mpdata_upload_book_failure AS a
 where to_timestamp(a.received_at) < (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_upload_book_failure AS b);
SELECT * from bloomreadertest.clone_upload_book_failure AS a;
SELECT * from bloomreadertest.mpdata_upload_book_failure AS a
