--DROP FUNCTION ACTUALmigrate_MPData_PrintPDF();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_PrintPDF()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_print_pdf AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.print_pdf AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.print_pdf as s  
	( id, received_at, browser, channel, command_line, context_library_name, 
	  context_library_version, culture, book_id,
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  public.empty_to_null(xyz_row.book_id), public.empty_to_null(xyz_row.current_directory), 
...
-- Column: bloomapp.print_pdf.location_uid
-- ALTER TABLE bloomapp.print_pdf DROP COLUMN location_uid;
ALTER TABLE bloomapp.print_pdf
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.print_pdf AS b

--SELECT * FROM ACTUALmigrate_MPData_PrintPDF();
select count(*) from bloomreadertest.clone_print_pdf;
