--DROP FUNCTION ACTUALmigrate_MPData_PublishAndroid();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_PublishAndroid()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_publish_android AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.publish_android AS b 
...
FOR xyz_row IN xyz
	...
	*/
	...
	END IF;
   	...
   	INSERT INTO bloomapp.publish_android as s  
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  public.empty_to_null(xyz_row.full_version), public.empty_to_null(xyz_row.language),
	  --xyz_row.ip,
...
ALTER TABLE bloomapp.publish_android
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_print_pdf AS b

--SELECT * FROM ACTUALmigrate_MPData_PublishAndroid();
...
select count(*) from bloomapp.publish_android;
select count(*) from bloomreadertest.mpdata_publish_android AS a
