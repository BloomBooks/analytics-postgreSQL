--DROP FUNCTION ACTUALmigrate_MPData_save_e_pub();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_save_e_pub()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_save_e_pub AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.save_e_pub AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.save_e_pub as s  
...
-- Column: bloomapp.save_e_pub .location_uid
-- ALTER TABLE bloomapp.save_e_pub DROP COLUMN location_uid;
ALTER TABLE bloomapp.save_e_pub 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.save_e_pub AS b

--SELECT * FROM ACTUALmigrate_MPData_save_e_pub();
select * from bloomreadertest.mpdata_save_e_pub
select to_timestamp('1522771253');
select count(*) from bloomreadertest.clone_save_e_pub;
select count(*) from bloomreadertest.clone_save_e_pub;
select count(*) from bloomreadertest.mpdata_save_e_pub AS a
 where to_timestamp(a.received_at) < (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_save_e_pub AS b);
SELECT * from bloomreadertest.clone_save_e_pub AS a;
SELECT * from bloomreadertest.mpdata_save_e_pub AS a
