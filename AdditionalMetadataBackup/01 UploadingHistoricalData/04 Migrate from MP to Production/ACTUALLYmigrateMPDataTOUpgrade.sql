--DROP FUNCTION ACTUALmigrate_MPData_upgrade();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_upgrade()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_upgrade AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.upgrade AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.upgrade as s  
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  public.empty_to_null(xyz_row.full_version), public.empty_to_null(xyz_row.old_version), 
	  xyz_row.ip,
...
-- Column: bloomapp.upgrade .location_uid
-- ALTER TABLE bloomapp.upgrade DROP COLUMN location_uid;
ALTER TABLE bloomapp.upgrade 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.upgrade AS b

--SELECT * FROM ACTUALmigrate_MPData_upgrade();
select * from bloomreadertest.mpdata_upgrade
select to_timestamp('1522771253');
select count(*) from bloomapp.upgrade;
select count(*) from bloomreadertest.clone_upgrade;
select count(*) from bloomreadertest.mpdata_upgrade AS a
 where to_timestamp(a.received_at) < (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_upgrade AS b);
SELECT * from bloomreadertest.clone_upgrade AS a;
SELECT * from bloomreadertest.mpdata_upgrade AS a
