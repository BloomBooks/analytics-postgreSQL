--DROP FUNCTION ACTUALmigrate_MPData_CreateNewSourceCollection();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_CreateNewSourceCollection()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_created_new_source_collection AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.created_new_source_collection AS b 
...
FOR xyz_row IN xyz
    ...
    IF country_code_temp = '-' THEN 
		IF public.empty_to_null(xyz_row.ip) IS NOT NULL THEN
			...
			END IF;	
		END IF;
		/*  WE DO NOT WANT TO USE THE SELF-ENTERED COUNTRY OF THE BOOK FOR LOCATION COUNTRY
		IF country_code_temp = '-' AND public.empty_to_null(xyz_row.country) <> '-' 
		                           AND public.empty_to_null(xyz_row.country) IS NOT NULL THEN
			SELECT DISTINCT (g.country_code) FROM public.ip2loc_sm_tab AS g 
			WHERE  UPPER(g.country_name) = UPPER(xyz_row.country)  INTO country_code_temp;
			IF country_code_temp ='-' or public.empty_to_null(country_code_temp) IS NULL THEN 
			    SELECT DISTINCT (h.country_code) FROM public.ipv42location AS h 
				WHERE  UPPER(h.country_name) = UPPER(xyz_row.country)  INTO country_code_temp;
	   			IF country_code_temp ='-' or public.empty_to_null(country_code_temp) IS NULL THEN
					country_code_temp := '-';
				END IF;
			END IF;	
	...
	END IF;
   	...
   	INSERT INTO  bloomapp.created_new_source_collection as s  
	( id, received_at, browser, channel, command_line, context_library_name, 
	  ...
	  working_set,
	  branding_project_name, collection_country, country, 
...
select * from bloomreadertest.mpdata_created_new_source_collection;

-- Column: bloomapp.created_new_source_collection.location_uid
-- ALTER TABLE bloomapp.created_new_source_collection.location_uid DROP COLUMN location_uid;
ALTER TABLE bloomapp.created_new_source_collection
    ADD COLUMN location_uid bigint;	
--SELECT * FROM ACTUALmigrate_MPData_CreateNewSourceCollection();

select * from bloomreadertest.clone_created_new_source_collection;
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_created_new_source_collection AS b
SELECT * from bloomreadertest.clone_created_new_source_collection AS a
   Where location_uid IS NOT NULL;
   
SELECT c.country_code, c.country_name, c.region, c.city 
FROM public.ip2loc_sm_tab AS c 
WHERE public.ip2int(COALESCE('41.190.212.202','0.0.0.0')) = c.context_ip;

SELECT f.country_code, f.country_name, f.region, f.city 
FROM public.ipv42location AS f WHERE public.ip2int(COALESCE('41.190.212.202','0.0.0.0')) 
BETWEEN f.ipv4_from AND f.ipv4_to
				
SELECT * from bloomreadertest.mpdata_create_new_vernacular_collection AS a
       WHERE to_timestamp(a.received_at) < 
	   (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_create_new_vernacular_collection AS b) 
	   ORDER BY a.mp_country, a.region, a.city;
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_create_new_vernacular_collection AS b	   
