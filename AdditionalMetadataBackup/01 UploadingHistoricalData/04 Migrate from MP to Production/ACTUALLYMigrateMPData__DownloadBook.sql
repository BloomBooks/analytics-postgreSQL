--DROP FUNCTION ACTUALmigrate_MPData_download_book();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_download_book()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_download_book AS a
...
SELECT MIN(b.timestamp) FROM bloomlibrary_org.download_book AS b 
...
SELECT * from bloomreadertest.mpdata_download_book AS a
...
--SELECT * FROM ACTUALmigrate_MPData_download_book();
select * from bloomreadertest.clone_download_book
select to_timestamp('1522771253');
select count(*) from bloomlibrary_org.download_book ;
select count(*) from bloomreadertest.clone_download_book;
