--DROP FUNCTION ACTUALmigrate_MPData_InsertTemplatePage();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_InsertTemplatePage()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_insert_template_page AS a
...
--SELECT MIN(b.timestamp) FROM bloomapp.insert_template_page AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.insert_template_page as s  
	( id, received_at, browser, channel, command_line, context_library_name, 
	  ...
	  page, template_source, -- collection_country, country, 
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	 ...
	 --public.empty_to_null(xyz_row.branding_project_name), 	
	  public.empty_to_null(xyz_row.page),
	  public.empty_to_null(xyz_row.template_source),
...
-- Column: bloomreadertest.clone_exported_xml_for_in_design.location_uid
-- ALTER TABLE bloomreadertest.clone_exported_xml_for_in_design DROP COLUMN location_uid;
ALTER TABLE bloomapp.insert_template_page 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_insert_template_page AS b

--SELECT * FROM ACTUALmigrate_MPData_InsertTemplatePage();
select count(*) from bloomreadertest.clone_insert_template_page;
select count(*) from bloomapp.insert_template_page;
select * from bloomreadertest.mpdata_insert_template_page AS a
