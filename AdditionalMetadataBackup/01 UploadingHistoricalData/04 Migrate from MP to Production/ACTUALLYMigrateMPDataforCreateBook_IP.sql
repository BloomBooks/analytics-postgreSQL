--DROP FUNCTION ACTUAL_migrate_MPDate_CreateBook();

CREATE OR REPLACE FUNCTION ACTUAL_migrate_MPDate_CreateBook()
...
SELECT MIN(b.timestamp) FROM bloomapp.create_book AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomapp.create_book as s  
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  loc_uid_temp );
     	--ON CONFLICT ON CONSTRAINT clone_create_book_pkey DO NOTHING;
...
-- Column: bloomapp.create_book.location_uid
-- ALTER TABLE bloomapp.create_book DROP COLUMN location_uid;
ALTER TABLE bloomapp.create_book
    ADD COLUMN location_uid bigint;	
	
--SELECT * FROM ACTUAL_migrate_MPDate_CreateBook();
...
select Count(*) from bloomreadertest.clone_create_book;
select * from bloomreadertest.clone_create_book;
select Count(*) from bloomreadertest.mpdata_create_book;
...
select * from bloomreadertest.clone_create_book
  WHERE command_line IS NOT NULL and location_uid Is not null;

select * from public.countryregioncitylu as p order by p.loc_uid DESC;



SELECT MIN(a.received_at) from bloomreadertest.clone_create_book AS a;
