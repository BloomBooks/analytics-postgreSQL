--DROP FUNCTION ACTUALmigrate_MPData_upload_book_success();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_upload_book_success()
...
SELECT MIN(b.timestamp) FROM bloomapp.upload_book_success AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.upload_book_success as s  
...
-- ALTER TABLE bloomapp.upload_book_success DROP COLUMN location_uid;
ALTER TABLE bloomapp.upload_book_success 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.upload_book_success AS b

--SELECT * FROM ACTUALmigrate_MPData_upload_book_success();
select * from bloomreadertest.mpdata_upload_book_success
select to_timestamp('1522771253');
select count(*) from bloomapp.upload_book_success;
