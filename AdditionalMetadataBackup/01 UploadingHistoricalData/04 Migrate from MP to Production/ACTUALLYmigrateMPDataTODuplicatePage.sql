--DROP FUNCTION ACTUALmigrate_MPData_DuplicatePage();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_DuplicatePage()
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.duplicate_page as s  
...
-- Column: bloomapp.duplicate_page.location_uid
-- ALTER TABLE bloomapp.duplicate_page DROP COLUMN location_uid;
ALTER TABLE bloomapp.duplicate_page
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.duplicate_page AS b

--SELECT * FROM ACTUALmigrate_MPData_DuplicatePage();
select count(*) from bloomapp.duplicate_page;
SELECT * from bloomapp.duplicate_page AS a;
