--DROP FUNCTION ACTUALmigrate_MPData_log_in();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_log_in()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_log_in AS a
...
SELECT MIN(b.timestamp) FROM bloomlibrary_org.log_in AS b 
...
FOR xyz_row IN xyz
	...
	(new_id, to_timestamp(xyz_row.received_at), 
	  ...
	  public.empty_to_null(xyz_row.username),
...
-- Column: bloomlibrary_org.log_in.location_uid
-- ALTER TABLE bloomlibrary_org.log_in DROP COLUMN location_uid;
ALTER TABLE bloomlibrary_org.log_in
    ADD COLUMN location_uid bigint;	
	
SELECT * from bloomreadertest.mpdata_log_in AS a
      WHERE to_timestamp(a.received_at) < '2017-08-07 09:57:22.415+00'	
SELECT MIN(b.timestamp) FROM bloomlibrary_org.log_in AS b
 
--SELECT * FROM ACTUALmigrate_MPData_log_in();

select * from bloomlibrary_org.log_in
  WHERE POSITION('Mix' IN context_library_name)>0;
select to_timestamp('1522771253');
select count(*) from bloomlibrary_org.log_in;
select count(*) from bloomlibrary_org.log_in;
...
SELECT * from bloomreadertest.mpdata_upload_book_success AS a
   	...
   	UPDATE bloomreadertest.mpdata_log_in
	SET  context_library_name = context_library_name||' via Mixpanel';
	
   	UPDATE bloomlibrary_org.log_in
	SET  context_library_name = context_library_name||' via Mixpanel'
	WHERE timestamp < '2017-08-07 13:29:54.165+00';
