--DROP FUNCTION ACTUALmigrate_MPData_Relocate_Page();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_Relocate_Page()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_relocate_page AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.relocate_page AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.relocate_page as s  
...
-- Column: bloomapp.relocate_page .location_uid
-- ALTER TABLE bloomapp.relocate_page  DROP COLUMN location_uid;
ALTER TABLE bloomapp.relocate_page 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.relocate_page  AS b

--SELECT * FROM ACTUALmigrate_MPData_Relocate_Page();
select * from bloomreadertest.mpdata_relocate_page
select to_timestamp('1522771253');
select count(*) from bloomapp.relocate_page;
select count(*) from bloomreadertest.clone_relocate_page;
select count(*) from bloomreadertest.mpdata_relocate_page AS a
 where to_timestamp(a.received_at) < (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_relocate_page AS b);
SELECT * from bloomreadertest.clone_relocate_page AS a;
SELECT * from bloomreadertest.mpdata_relocate_page AS a
       WHERE to_timestamp(a.received_at) < 
	   (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_relocate_page AS b) 
	   ORDER BY a.mp_country, a.region, a.city;

select * from bloomapp.create_relocate_page;
