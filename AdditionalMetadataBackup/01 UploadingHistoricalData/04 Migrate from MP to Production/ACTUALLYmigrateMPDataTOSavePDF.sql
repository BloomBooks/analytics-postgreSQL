--DROP FUNCTION ACTUALmigrate_MPData_save_pdf();

CREATE OR REPLACE FUNCTION ACTUALmigrate_MPData_save_pdf()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_save_pdf AS a
...
SELECT MIN(b.timestamp) FROM bloomapp.save_pdf AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO bloomapp.save_pdf as s  
...
-- Column: bloomapp.save_pdf .location_uid
-- ALTER TABLE bloomapp.save_pdf DROP COLUMN location_uid;
ALTER TABLE bloomapp.save_pdf 
    ADD COLUMN location_uid bigint;	
	
SELECT MIN(b.timestamp) FROM bloomapp.save_pdf AS b

--SELECT * FROM ACTUALmigrate_MPData_save_pdf();
select * from bloomreadertest.mpdata_save_pdf
select to_timestamp('1522771253');
select count(*) from bloomreadertest.clone_save_pdf;
select count(*) from bloomreadertest.clone_save_pdf;
select count(*) from bloomreadertest.mpdata_save_pdf AS a
 where to_timestamp(a.received_at) < (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_save_pdf AS b);
SELECT * from bloomreadertest.clone_save_pdf AS a;
SELECT * from bloomreadertest.mpdata_save_pdf AS a
       WHERE to_timestamp(a.received_at) < 
	   (SELECT MIN(b.timestamp) FROM bloomreadertest.clone_save_pdf AS b) 
	   ORDER BY a.mp_country, a.region, a.city;

select * from bloomapp.create_save_pdf;
