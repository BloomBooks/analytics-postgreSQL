--DROP FUNCTION migrate_MPData_Created();

CREATE OR REPLACE FUNCTION migrate_MPData_Created()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_created AS a
...
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_created AS b 
...
FOR xyz_row IN xyz
    ...
    IF country_code_temp = '-' THEN 
		IF public.empty_to_null(xyz_row.ip) IS NOT NULL THEN
			...
			END IF;	
		END IF;
		/*  THERE IS NO country column in the mpdata
	...
	END IF;
   	...
   	INSERT INTO  bloomreadertest.clone_created as s  
...
-- Column: bloomapp.clone_created.location_uid
-- ALTER TABLE bloomapp.clone_created DROP COLUMN location_uid;
ALTER TABLE bloomreadertest.clone_created
    ADD COLUMN location_uid bigint;	
--SELECT * FROM migrate_MPData_Created();

select * from bloomapp.created;
SELECT MIN(b.timestamp) FROM bloomapp.created AS b
SELECT * from bloomreadertest.clone_created AS a
