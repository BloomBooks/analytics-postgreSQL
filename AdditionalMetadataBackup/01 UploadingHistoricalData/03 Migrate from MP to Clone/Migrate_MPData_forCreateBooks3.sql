--DROP FUNCTION migrate_MPData2x_CreateBooks();

CREATE OR REPLACE FUNCTION migrate_MPData2x_CreateBooks()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_create_book AS a
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomreadertest.clone2_create_book as s  
	...
	(new_id, to_timestamp(xyz_row.received_at), public.empty_to_null(xyz_row.browser), 
	  ...
	  loc_uid_temp );
     	--ON CONFLICT ON CONSTRAINT clone2_change_picture_pkey DO NOTHING;
...
--SELECT * FROM migrate_MPData2x_CreateBooks();

select count(*) from bloomapp.create_book;
SELECT * from bloomapp.create_book AS a
  WHERE location_uid = 1;
SELECT a.country,a.ip from bloomreadertest.clone2_create_book AS a
  WHERE location_uid = 1;
SELECT * from bloomreadertest.mpdata_create_book AS a
       WHERE to_timestamp(a.received_at) < 
	   (SELECT MIN(b.timestamp) FROM bloomreadertest.clone2_create_book AS b) 
	   ORDER BY a.mp_country, a.region, a.city;
SELECT MIN(b.timestamp) FROM bloomreadertest.clone2_create_book AS b	   
select MAX(to_timestamp(a.received_at)) from bloomreadertest.mpdata_create_book AS a
...
SELECT * from bloomreadertest.clone2_create_book AS a
