--DROP FUNCTION migrate_MPData_DuplicatePage();

CREATE OR REPLACE FUNCTION migrate_MPData_DuplicatePage()
...
DECLARE xyz CURSOR FOR SELECT * from bloomreadertest.mpdata_duplicate_page AS a
...
SELECT MIN(b.timestamp) FROM bloomreadertest.clone_duplicate_page AS b 
...
FOR xyz_row IN xyz
	...
	END IF;
   	...
   	INSERT INTO  bloomreadertest.clone_duplicate_page as s  
...
SELECT MIN(b.timestamp) FROM bloomapp.duplicate_page AS b

--SELECT * FROM migrate_MPData_DuplicatePage();
select count(*) from bloomreadertest.clone_duplicate_page;
