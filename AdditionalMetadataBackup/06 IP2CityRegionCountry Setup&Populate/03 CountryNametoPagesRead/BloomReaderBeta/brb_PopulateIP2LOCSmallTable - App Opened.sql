...
DECLARE xyz CURSOR FOR SELECT * from bloomreaderbeta.application_opened AS a;
...
FOR xyz_row IN xyz
	...
	SELECT b.country_code, b.country_name FROM public.ip2location AS b 
           WHERE (SELECT public.ip2int(d.context_ip) FROM bloomreaderbeta.application_opened AS d 
...
select * from bloomreaderbeta.application_opened where populate_country is not null;
select * from bloomreaderbeta.application_opened;

select * FROM bloomreaderbeta.application_opened AS a WHERE a.id ='010a3627-2357-43be-b383-6ce518961eae'; 


SELECT b.populate_country FROM public.ip2location AS b
   WHERE (SELECT ip2int(a.context_ip) FROM bloomreaderbeta.application_opened AS a 
                 WHERE a.id = '010a3627-2357-43be-b383-6ce518961eae') BETWEEN b.ip_from and b.ip_to;
		
UPDATE bloomreaderbeta.application_opened AS a 
